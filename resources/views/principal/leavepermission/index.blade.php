...
<div class="min-h-screen bg-blue-200 py-3">
  ...
  <h1 class="mb-3 mt-12 text-center text-2xl text-black font-bold">Pengajuan Cuti Saya</h1>
  <div class="w-40">
    <div class="my-2 flex sm:flex-row flex-col">
      <div class="block relative">
        <a class="text-white" href="/principal/myleavepermission/create">
  ...
  <div class="w-full overflow-x-auto">
    <table class="w-full">
      ...
      <tbody class="bg-white">
        ...
        <tr class="text-gray-700 text-center">
          <td class="px-4 py-3 text-ms border font-semibold">{{$loop->iteration}}</td>
          <td class="px-4 py-3 text-ms border">{{$mydata->leaveType->name}}</td>
          <td class="px-4 py-3 text-ms border">{{$mydata->leave_duration}} Hari</td>
          <td class="px-4 py-3 text-ms border">{{$mydata->end_date->formatLocalized("%d/%m/%Y")}}</td>
          <td class="px-4 py-3 text-ms border">{!! $mydata->leave_excuse !!}</td>
          @if($mydata->is_direct_supervisor_approve == FALSE)
          ...
          @elseif($mydata->is_direct_supervisor_approve == TRUE && $mydata->is_official_approve == FALSE && $mydata->is_rejected == FALSE)
          <td class="px-4 py-3 text-sm border">
